...
export const getSingleBookmark = async (req, res) => {
  ...
  const bookmarkedArticle = await Bookmarks.findOne({ where: { id: bookmarkId, userId } });

  if (!bookmarkedArticle) return res.status(404).json({ status: 'failure', error: 'bookmarked article does not exist' });
...
export const deleteBookmark = async (req, res) => {
  ...
  return res.status(200).json({ status: 'success', message: 'the article has been removed from bookmark successfully' });
};
